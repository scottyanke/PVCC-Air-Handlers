    ...
    for(;;)     // this program loops forever
    {
        ...
        {   // going into this routine means we got an escape character, and at least one other character
            ...
            if (rcv_buffer[0] == myId)  // only answer our own id
            {
                ...
                AMux_Select(0); // select the first analog line
                PSI_SetOffset(40000);   // again, not sure if it's needed
                CyDelay(50);    // let things settle...
                PSI = PSI_Read32(); // read the ADC
                fPSI = (PSI_CountsTo_mVolts(PSI) / 9) * .1450377377;    // convert the voltage to PSI
                if (fPSI < 0.1)
                    fPSI = 0.0;
                ...
                CyDelay(50);
                PSI = PSI_Read32();
                fPSI = (PSI_CountsTo_Volts(PSI) - 2.75)  * .1450377377; // convert voltage to PSI with a different offset
                if (fPSI < 0.0)
                    fPSI = 0.0;
                sprintf(s,"%c:A:%05.3lf:%06d:\r\n",myId,fPSI,PSI_CountsTo_mVolts(PSI));
                RS485_PutString(s);
                CyDelay(150);
                ...
                for (i = 0; i < 8; i++)     // get the temperatures from up to 8 MAX31865 devices
                {
